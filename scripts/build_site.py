...
def pizza_calculator() -> str:
    return '''<section class="pizza-calculator" id="degkalkylator"><h2>Degkalkylator</h2><p>Välj hur många pizzor du vill göra och hur stor poolish du redan har gjort. Receptet räknar med 70 % hydrering och 4 % salt.</p><div class="calc-inputs"><label>Antal pizzor<input id="pizza-count" type="number" min="1" max="30" step="1" value="6"></label><label>Gram per degboll<input id="ball-weight" type="number" min="150" max="500" step="5" value="250"></label><label>Poolish – mjöl + vatten<select id="poolish-size"><option value="300">300 + 300 g</option><option value="400">400 + 400 g</option><option value="500" selected>500 + 500 g</option></select></label></div><div id="calc-warning" class="calc-warning" role="status"></div><div class="calc-result"><p><strong>Total deg:</strong> <span id="total-dough"></span> g</p><h3>Tillsätt nästa dag</h3><table><tbody><tr><th>Caputo Pizzeria-mjöl</th><td><span id="add-flour"></span> g</td></tr><tr><th>Vatten</th><td><span id="add-water"></span> g</td></tr><tr><th>Salt</th><td><span id="add-salt"></span> g</td></tr></tbody></table></div><p class="calc-note">Poolish: lika delar mjöl och vatten + 5 g jäst. Jästen ingår i totalvikten i beräkningen.</p></section><script>(function(){const count=document.getElementById('pizza-count'),weight=document.getElementById('ball-weight'),pool=document.getElementById('poolish-size'),warn=document.getElementById('calc-warning');function calc(){let n=Math.max(1,Math.min(30,Number(count.value)||1)),w=Math.max(150,Math.min(500,Number(weight.value)||250)),p=Number(pool.value),target=n*w,yeast=5,flour=(target-yeast)/1.74,water=.70*flour,salt=.04*flour,af=flour-p,aw=water-p;document.getElementById('total-dough').textContent=Math.round(target);document.getElementById('add-flour').textContent=Math.max(0,Math.round(af));document.getElementById('add-water').textContent=Math.max(0,Math.round(aw));document.getElementById('add-salt').textContent=Math.round(salt);warn.textContent=(af<0||aw<0)?'Den valda poolishen är för stor för den här degmängden. Välj en mindre poolish eller gör fler pizzor.':'';}[count,weight,pool].forEach(x=>x.addEventListener('input',calc));calc();})();</script>'''
